...
def tokenize_trajectory(data):
    ...
    v = data[:, :, 9:12]         # vx, vy, vz
    av = data[:, :, 12:15]       # avx, avy, avz
    strand_index = data[:, :, 15]
    base = data[:, :, 16]
    neighbor_3 = data[:, :, 17]
    neighbor_5 = data[:, :, 18]
    # energies = data[:, :, 19:]
    ...
    nodes = torch.cat([
        ...
        neighbor_3.unsqueeze(2), neighbor_5.unsqueeze(2), v, av
    ], dim=2)
    ...
    # edge_attr_angles = np.zeros_like(edge_attr_distances)
